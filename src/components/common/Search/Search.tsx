...
export const Search: React.FC<SearchPropsType> = ({friends,addTochedUser}) => {
    ...
    const searchResult = (text: string) => {
        ...
        return result.map(r => {
            ...
            return({
            value: r.name,
            label: <NavLink style={{color:"black"}} onClick={addTochedUserHandler} to={'/dialog/' + r.id} key={r.id}>
                        <Row  align={"middle"}>

                            <img className={sCommentForm.avatar} src={avatar} alt="avatar"/> <span style={{paddingLeft:5}}> {r.name}</span>
                        </Row>
            </NavLink>
        })})
    }
